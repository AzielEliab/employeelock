...
- Suite mesh proxy: [https://employeelock-download-tracker.vibelock.workers.dev/v1/mesh](https://employeelock-download-tracker.vibelock.workers.dev/v1/mesh) — default OFF; QNM live / locked / isolated; QNS-CD-1.0 photon QNS1 cross-map (not a Softwares-tab product; no public qnsd proxy)
...
- `GET /v1/mesh` — PROXY suite mesh status (default OFF; QNM live|locked|isolated; QNS-CD-1.0 photon QNS1 packet transfer cross-map)
...
- MCP: catalog `https://aziel-runtime.vibelock.workers.dev/mcp` and this Worker `/mcp`. Suite mesh `/v1/mesh/*` PROXY via `AZIEL_RUNTIME` (default OFF; QNM-BUILD-1.0 live|locked|isolated; QNS-CD-1.0 photon QNS1 packet transfer cross-map; no Node Gate; no public qnsd proxy). Catalog MCP `mesh_*` + FragGate `slug=mesh`. Hub cite / Worker mesh cross-map only — not a Softwares-tab product. Local qnsd is coded in [qnm-node](https://github.com/AzielEliab/qnm-node). Runtime cites + catalog field live in [aziel-runtime](https://github.com/AzielEliab/aziel-runtime). [AZInterface](https://github.com/AzielEliab/azinterface) has pair custody.
